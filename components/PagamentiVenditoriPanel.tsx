...
import { SectionCard, Skeleton, EmptyState, ErrorBox, colors, S, euro } from '@/components/ui'
...
/** Pannello gestione pagamenti venditori: fee dovuta, già pagato, da pagare.
    Ogni riga apre il dettaglio dove il CEO registra un pagamento. */
export default function PagamentiVenditoriPanel() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([cfg, vendite, pagamenti]) => {
        const nomi: string[] = (cfg.venditori || [])
  ...
  const totDaPagare = venditori.reduce((s, v) => s + Math.max(v.daPagare, 0), 0)

  return (
    <SectionCard
      title="Pagamenti venditori"
      subtitle={loading ? 'Caricamento…' : `${euro(totDaPagare)} ancora da pagare in totale`}
    >
